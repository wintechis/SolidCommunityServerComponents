...
describe('A BasePasswordStore', (): void => {
  ...
  const normalized = encodeURIComponent('test@example.com');
  ...
  beforeEach(async(): Promise<void> => {
    payload = { id, email: normalized, accountId, password: await hash(password, 10), verified: true };
  ...
  it('can create logins.', async(): Promise<void> => {
    ...
    expect(storage.find).toHaveBeenLastCalledWith(STORAGE_TYPE, { email: normalized });
    expect(storage.create).toHaveBeenCalledTimes(1);
    expect(storage.create).toHaveBeenLastCalledWith(STORAGE_TYPE,
      { accountId, verified: false, email: normalized, password: expect.any(String) });
  });
