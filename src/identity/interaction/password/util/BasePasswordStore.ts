...
export class BasePasswordStore extends Initializer implements PasswordStore {
  ...
  protected encodeEmail(email: string): string {
    return encodeURIComponent(email).toLowerCase();
  }

  protected decodeEmail(email: string): string {
  ...
  public async get(id: string): Promise<{ email: string; accountId: string } | undefined> {
    ...
    return { email: this.decodeEmail(result.email), accountId: result.accountId };
  }

  public async findByEmail(email: string): Promise<{ accountId: string; id: string } | undefined> {
    const payload = await this.storage.find(STORAGE_TYPE, { email: this.encodeEmail(email) });
    if (payload.length === 0) {
      return;
    }
  ...
  public async findByAccount(accountId: string): Promise<{ id: string; email: string }[]> {
    return (await this.storage.find(STORAGE_TYPE, { accountId }))
      .map(({ id, email }): { id: string; email: string } => ({ id, email: this.decodeEmail(email) }));
  }
